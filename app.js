...
const fallbackTables = [
  { id: null, table_number: "01", capacity: 4, zone: "Sank", map_left: "8%", map_top: "12%", is_vip: false },
  { id: null, table_number: "02", capacity: 4, zone: "Sank", map_left: "31%", map_top: "8%", is_vip: false },
  { id: null, table_number: "03", capacity: 6, zone: "Sprat", map_left: "58%", map_top: "12%", is_vip: true },
  { id: null, table_number: "04", capacity: 4, zone: "Sank", map_left: "80%", map_top: "18%", is_vip: false },
  { id: null, table_number: "05", capacity: 6, zone: "Lounge", map_left: "18%", map_top: "49%", is_vip: false },
  { id: null, table_number: "06", capacity: 8, zone: "VIP", map_left: "45%", map_top: "42%", is_vip: true },
  { id: null, table_number: "07", capacity: 4, zone: "Lounge", map_left: "73%", map_top: "53%", is_vip: false },
  { id: null, table_number: "08", capacity: 5, zone: "Sprat", map_left: "10%", map_top: "76%", is_vip: false },
  { id: null, table_number: "09", capacity: 6, zone: "Sprat", map_left: "61%", map_top: "78%", is_vip: false },
];
...
function renderTables() {
  tableMap.innerHTML = tables
    .map(
      (table) => `
        <button
          class="table ${table.is_vip ? "vip" : ""}"
          type="button"
          style="left:${escapeHtml(table.map_left || "10%")};top:${escapeHtml(table.map_top || "10%")}"
          data-table="${escapeHtml(table.table_number)}"
          aria-label="Sto ${escapeHtml(table.table_number)}, ${escapeHtml(table.capacity)} mesta, ${escapeHtml(table.zone)}${
            table.is_vip ? ", VIP" : ""
          }"
        >
          <strong>${escapeHtml(table.table_number)}</strong>
          <span>${table.is_vip ? "VIP" : `${escapeHtml(table.capacity)} mesta`}</span>
        </button>
      `,
    )
    .join("");
      ...
      (pkg) => `
        <article class="package-card" tabindex="0" role="button" data-package="${escapeHtml(pkg.name)}">
          <div class="package-top">
            <div>
              <p class="eyebrow">Bottle service</p>
  ...
  return `
    <div class="modal-stack">
      ...
      <form class="modal-form two" data-booking-form data-mode="${escapeHtml(mode)}">
        ...
        <label>Broj gostiju<input name="guests" type="number" min="1" max="12" value="4" required /></label>
...
async function loadSupabaseData() {
  try {
    ...
    if (databaseTables.length) tables = databaseTables;
    renderEvents();
    renderTables();
  } catch (error) {
    console.error(error);
...
tableMap.addEventListener("click", (event) => {
  ...
  if (!button) return;
  selectedTable = tables.find((table) => table.table_number === button.dataset.table);
  document.querySelectorAll(".table").forEach((table) => table.classList.remove("selected"));
  button.classList.add("selected");
  selectedTitle.textContent = `Sto ${selectedTable.table_number}`;
  selectedCopy.textContent = `${selectedTable.zone} zona, ${selectedTable.capacity} mesta${
...
reserveSelected.addEventListener("click", () => {
  ...
  openModal(
    bookingForm({
      mode: "reservation",
      title: `Sto ${selectedTable.table_number}`,
      intro: `${selectedTable.zone} zona za ${selectedTable.capacity} gostiju.`,
